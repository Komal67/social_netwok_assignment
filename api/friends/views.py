...
class ListFriendsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        friends = Friend.objects.filter(user=user).values('friend')
...
class PendingFriendRequestListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        pending_friend_requests = self.get_queryset()
        all_pending_requests = []

        for pending_friend_request in pending_friend_requests:
            requester = User.objects.get(id=pending_friend_request.requester_id)

            pending_request_data = {
                'id': pending_friend_request.id,
